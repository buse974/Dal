...
class ResultSetTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $mock_to_array = $this->getMockBuilder('toArray')
                              ->setMethods(array('toArray'))
                              ->getMock();

        $mock_to_array->expects($this->once())
                      ->method('toArray')
                      ->will($this->returnValue(array('key' => 'ktata', 'arr' => 'value')));
    ...
    public function testToArrayCurrent()
    {
        ...
        $m_model->expects($this->exactly(0))
                ->method('toArrayCurrent')
                ->will($this->returnValue(array('arraycurent')));

        $m_model->expects($this->exactly(1))
                ->method('toArray')
                ->will($this->returnValue(array('id' => 'tata', 'value' => 'vtata')));

        $arr->append($m_model);

        $m_model = $this->getMockBuilder('MockArrayCopy')
        ...
        $this->assertEquals(array('value' => 'vtata'), $out['tata']);
    ...
    public function testToArrayCurrentWihtoutIndice()
    {
        ...
        $this->assertEquals(array('id' => 'tata', 'value' => 'vtata'), $out[1]);
        $this->assertEquals(array('id' => 'titi', 'value' => 'vtiti'), $out[2]);
    }

    public function testToArrayCurrentException()
    {
        $this->setExpectedException(
                'RuntimeException', 'Rows as part of this DataSource, with type object cannot be cast to an array Current', 0
        );
