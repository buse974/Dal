...
class TableGatewayTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        $m_resultset = $this->getMockBuilder('Dal\Db\ResultSet\ResultSet')->setMethods(['initialize'])->getMock();
        $m_resultset->expects($this->once())->method('initialize')->will($this->returnSelf());

        $m_statement = $this->getMockObjectGenerator()->getMock('Zend\Db\Adapter\Driver\Pdo\Statement', array('execute'));

        $m_adapter = $this->getMockBuilder('Dal\Db\TableGateway\TableGateway')
    ...
    {
        $m_adapter = $this->getMockBuilder('stdClass')
            ->setMethods(['query', 'execute', 'count', 'getDriver', 'getLastGeneratedValue', 'getResource', 'closeCursor'])
            ->getMock();
        ...
        $m_adapter->expects($this->once())->method('closeCursor')->will($this->returnSelf());
    ...
    {
        $m_constraint = $this->getMockBuilder('Zend\Db\Metadata\Object\ConstraintKeyObject')
            ->disableOriginalConstructor()
